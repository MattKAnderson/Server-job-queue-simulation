...
/*
	A class to model an M/M/N server system and compute the expected number
	of jobs waiting in the system. 
*/
class simulation {
    public:
        simulation(int num_servers, 
		           double arrival_mean, 
				   double process_mean, 
				   int q_part);
		simulation(const simulation &cp): simulation(cp.t_process.size(), 
		                                             cp.a_mean, 
													 cp.p_mean, 
													 cp.part) {};

...
int main(){
	...
	const int sim_time = static_cast<const int>(a_mean) * 50000;
	...
	//each thread gets a copy of the simulation object
	#pragma omp parallel for firstprivate(part_1) 
	for (int i = 0; i < total_reps; i++) {
		part_1.advance(sim_time);
		jobs_queued[i] = part_1.return_jobs_waiting();
	}
	std::cout << "The mean number of jobs waiting in part 1 is: " 
				<< mean_of_ints(jobs_queued) << std::endl;

	//simulation for part 2
	omp_set_num_threads(num_threads);
	omp_set_dynamic(0);
	#pragma omp parallel for firstprivate(part_2) 
	for (int i = 0; i < total_reps; i++) {
		part_2.advance(sim_time);
		jobs_queued[i] = part_2.return_jobs_waiting();
	}
	std::cout << "The mean number of jobs waiting in part 2 is: " 
				<< mean_of_ints(jobs_queued) << std::endl;
}
